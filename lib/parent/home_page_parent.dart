...
class Home_Page_Parent extends StatefulWidget {
  const Home_Page_Parent({Key? key}) : super(key: key);

  @override
  _Home_PageState createState() => _Home_PageState();
}

class _Home_PageState extends State<Home_Page_Parent> {
  ...
  void _onItemTapped(int index) {
    if (index == 1) { // Check if the Profile tab is clicked (index 1)
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => const Parent_Profile()),
      );
    } else {
      setState(() {
